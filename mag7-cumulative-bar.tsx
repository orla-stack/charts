...
const raw = {
  Alphabet:  [10096269997,3731720235,7471859998,14280047098,18253458672],
  Nvidia:    [371499998,421849998,5270063329,25745596663,35035299996],
  Microsoft: [6204765658,2886733331,14183741662,11134999998,41898299999],
  Tesla:     [0,0,0,0,0],
  Meta:      [876999999,48999999,0,11003100000,0],
  Amazon:    [8109861599,832229998,5454376998,6682790096,2543854246],
  Apple:     [579899997,0,0,0,0],
};
...
// Top to bottom order
const ORDER = ['Microsoft','Nvidia','Alphabet','Amazon','Meta','Apple','Tesla'];

const data = ORDER.map(name => ({
  name,
  value: parseFloat((raw[name].reduce((s, v) => s + v, 0) / 1e9).toFixed(1)),
  color: COLORS[name],
}));
...
const themes = {
  ...
  light: {
    ...
    border: 'rgba(0,0,0,0.08)', grid: 'rgba(0,0,0,0.06)',
...
export default function Mag7CumulativeBar() {
      ...
      <div style={{ maxWidth: 900, width: '100%' }}>
        <div style={{ background: t.card, borderRadius: 16, border: `1px solid ${t.border}`, padding: '40px 48px 36px', transition: 'all 0.3s ease' }}>
          ...
          <h2 style={{ color: t.text, fontSize: 38, fontWeight: 900, letterSpacing: '-0.03em', lineHeight: 1.1, margin: 0, fontFamily: font }}>
            (Some of) The Magnificent 7 Are Betting Tens of Billions on Startups
          </h2>
          <p style={{ color: t.textSec, fontSize: 13, margin: '10px 0 32px', fontFamily: font }}>
            Cumulative value of VC deals participated in · 2021–2025
          </p>

          {/* Custom SVG bar chart */}
          <div style={{ width: '100%', overflowX: 'auto' }}>
            <svg width="100%" viewBox={`0 0 ${labelW + chartW + valueW + 16} ${totalH}`} style={{ fontFamily: font }}>
              {data.map((d, i) => {
                const y = i * (barH + gap);
                ...
                return (
                  <g key={d.name}>
                    {/* Label */}
                    <text
                      x={labelW - 10} y={y + barH / 2 + 5}
                      textAnchor="end" fill={t.textSec}
                      fontSize={13} fontWeight={600}
                    >
                      {d.name}
                    </text>
                    {/* Background track */}
                    <rect x={labelW} y={y} width={chartW} height={barH} fill={t.barBg} rx={4} />
                    {/* Bar */}
                    <rect x={labelW} y={y} width={barW} height={barH} fill={d.color} rx={4} opacity={0.9} />
                    {/* Value label */}
                    <text
                      x={labelW + barW + 10} y={y + barH / 2 + 5}
                      fill={d.color} fontSize={13} fontWeight={700}
                    >
                      {fmt(d.value)}
                    </text>
                  </g>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 28, paddingTop: 16, borderTop: `1px solid ${t.footerBorder}` }}>
