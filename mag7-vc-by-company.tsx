...
const raw = {
  ...
  Alphabet:  [792009998,464799998,213650000,311339999,1131199997,2552100000,2650949992,1679106021,6952226024,1755799998,6116649996,10096269997,3731720235,7471859998,14280047098,18253458672,19613499996],
  Nvidia:    [0,0,0,0,0,2699999,35299999,322579997,142399998,126699999,42999999,371499998,421849998,5270063329,25745596663,35035299996,54455000000],
  Microsoft: [49500000,60999999,316399999,53389997,23559999,1703029997,978210000,2086909000,749265866,3400659997,1757096501,6204765658,2886733331,14183741662,11134999998,41898299999,30341999998],
  Tesla:     [0,0,0,0,0,0,0,0,55000000,0,0,0,0,0,0,0,20000000000],
  Meta:      [0,0,0,1700000,0,10560999,40000,0,1000000,40000,6014999999,876999999,48999999,0,11003100000,0,49999999],
  Amazon:    [202749997,611519998,12199999,116999998,44999999,254199995,259299998,714300000,1370061850,3272873331,3041139999,8109861599,832229998,5454376998,6682790096,2543854246,24733333],
  Apple:     [0,0,5000000,0,0,0,5500000000,0,0,499999,0,579899997,0,0,0,0,0],
};
...
const companies = Object.keys(COLORS);
const toB = v => parseFloat((v / 1e9).toFixed(3));

const chartData = raw.years.slice(0, 16).map((yr, i) => {
  const d = { year: String(yr) };
  companies.forEach(c => { d[c] = toB(raw[c][i]); });
  return d;
});
...
const themes = {
  dark: {
    ...
    tooltipBg: 'rgba(0,18,52,0.97)', tooltipBorder: 'rgba(255,255,255,0.1)', tooltipText: '#FFFFFF',
    ...
    badgeBg: 'rgba(62,236,210,0.12)', badgeColor: '#3EECD2',
  },
  light: {
    bg: '#F1F4F8', card: '#FFFFFF', text: '#0F172A',
    textSec: '#475569', textMuted: '#94A3B8',
    grid: 'rgba(0,0,0,0.06)', border: 'rgba(0,0,0,0.08)',
    tooltipBg: '#FFFFFF', tooltipBorder: 'rgba(0,0,0,0.1)', tooltipText: '#0F172A',
    ...
    badgeBg: 'rgba(13,148,136,0.1)', badgeColor: '#0D9488',
  },
};
...
const CustomTooltip = ({ active, payload, label, t }) => {
  ...
  return (
    <div style={{ background: t.tooltipBg, border: `1px solid ${t.tooltipBorder}`, borderRadius: 10, padding: '12px 16px', fontFamily: font, fontSize: 13, color: t.tooltipText, boxShadow: '0 8px 32px rgba(0,0,0,0.2)', minWidth: 180 }}>
      ...
      {sorted.map(p => (
        <div key={p.dataKey} style={{ display: 'flex', justifyContent: 'space-between', gap: 16, marginBottom: 4 }}>
          <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            <span style={{ color: t.textSec }}>{p.dataKey}</span>
          </span>
...
// Custom dot: only renders at the last data point, draws dot + label
const EndDot = ({ cx, cy, index, color, name }) => {
  if (index !== lastIdx || cy == null || isNaN(cy)) return null;
  return (
    <g>
      <circle cx={cx} cy={cy} r={3} fill={color} />
      <text x={cx + 8} y={cy + 4} fill={color} fontSize={11} fontWeight={700} fontFamily={font}>{name}</text>
    </g>
  );
};
...
export default function Mag7ByCompany() {
  const [mode, setMode] = useState('dark');
  const t = themes[mode];

  return (
    <div style={{ background: t.bg, minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '32px 24px', fontFamily: font, transition: 'all 0.3s ease' }}>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Source+Sans+3:wght@400;500;600;700;900&display=swap');`}</style>
      <div style={{ maxWidth: 960, width: '100%' }}>
        <div style={{ background: t.card, borderRadius: 16, border: `1px solid ${t.border}`, padding: '40px 48px 36px', transition: 'all 0.3s ease' }}>

          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 14 }}>
            <span style={{ background: t.badgeBg, color: t.badgeColor, padding: '4px 12px', borderRadius: 20, fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.07em' }}>
              🏦 Big Tech Goes VC
            </span>
          ...
          <h2 style={{ color: t.text, fontSize: 38, fontWeight: 900, letterSpacing: '-0.03em', lineHeight: 1.1, margin: 0, fontFamily: font }}>
            The Magnificent 7 Ramping Up Startup Investment Activity
          </h2>
          <p style={{ color: t.textSec, fontSize: 13, margin: '10px 0 28px', fontFamily: font }}>
            Total value of VC deals participated in ($B) · 2010–2025
          </p>

          <ResponsiveContainer width="100%" height={380}>
            <LineChart data={chartData} margin={{ top: 10, right: 110, bottom: 20, left: 10 }}>
              <CartesianGrid stroke={t.grid} vertical={false} />
              <XAxis dataKey="year" tick={{ fill: t.textSec, fontSize: 11, fontFamily: font, fontWeight: 500, angle: -35, textAnchor: 'end' }} axisLine={{ stroke: t.grid }} tickLine={false} interval={0} height={52} />
              <YAxis tick={{ fill: t.textMuted, fontSize: 11, fontFamily: font }} axisLine={false} tickLine={false} tickFormatter={v => `$${v}B`} width={50} />
              ...
              {companies.map(c => (
                <Line
                  ...
                  stroke={COLORS[c]}
                  strokeWidth={2}
                  dot={(props) => <EndDot {...props} color={COLORS[c]} name={c} />}
                  activeDot={{ r: 4, fill: COLORS[c] }}
                  connectNulls={false}
                />
              ))}
            </LineChart>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 24, paddingTop: 16, borderTop: `1px solid ${t.footerBorder}` }}>
