...
const data = [
  { year: 2010, company: 'Winamax', value: 11 },
  { year: 2011, company: 'Prototool Mfg', value: 10 },
  { year: 2012, company: 'Scanbot', value: 10 },
  { year: 2013, company: 'DE Group', value: 49 },
  { year: 2014, company: 'Gamma Asset F.', value: 55 },
  { year: 2015, company: 'Tingbot', value: 39 },
  { year: 2016, company: 'Sirin Labs', value: 72 },
  { year: 2017, company: 'Synswap', value: 21 },
  { year: 2018, company: 'Cazoo', value: 40 },
  { year: 2019, company: 'Health Hero', value: 64 },
  { year: 2020, company: 'Aidexa', value: 41 },
  { year: 2021, company: 'Newcleo', value: 118 },
  { year: 2022, company: 'Carmoola', value: 36 },
  { year: 2023, company: 'Mistral AI', value: 95 },
  { year: 2024, company: 'H Company', value: 80 },
  { year: 2025, company: 'General Intuition', value: 134 },
...
const CustomTooltip = ({ active, payload, t }) => {
  ...
  return (
    <div style={{
      ...
      borderRadius: 10, padding: '12px 16px', color: t.tooltipText,
      fontSize: 13, fontFamily: "'Source Sans 3', system-ui",
      boxShadow: '0 8px 32px rgba(0,0,0,0.15)',
    }}>
      <div style={{ fontWeight: 700, fontSize: 15, marginBottom: 6 }}>{d.company}</div>
      <div style={{ color: t.textSec, marginBottom: 2 }}>Year: <span style={{ color: t.text, fontWeight: 600 }}>{d.year}</span></div>
      <div style={{ color: t.textSec }}>Seed round: <span style={{ color: d.value >= 200 ? '#EF4444' : t.accent, fontWeight: 700, fontSize: 15 }}>${d.value}M</span></div>
    </div>
  );
};

// Custom dot rendered via SVG inside the chart
const CustomDot = ({ cx, cy, payload, t, hovered }) => {
  const isRecord = payload.value === 1000;
  const isHovered = hovered === payload.year;
  const dimmed = hovered && !isHovered;
  const color = isRecord ? '#EF4444' : t.accent;
  // bubble size scaled by value, capped
  const minR = 7, maxR = 38;
  const minV = 10, maxV = 1000;
  const r = minR + ((payload.value - minV) / (maxV - minV)) * (maxR - minR);

  return (
    <g>
      <circle cx={cx} cy={cy} r={r}
        fill={color} fillOpacity={dimmed ? 0.15 : isHovered ? 0.35 : 0.18}
        stroke={color} strokeWidth={dimmed ? 1 : 2}
        strokeOpacity={dimmed ? 0.25 : 1}
        style={{ transition: 'all 0.2s ease', cursor: 'pointer' }}
      />
      <circle cx={cx} cy={cy} r={5}
        fill={color} fillOpacity={dimmed ? 0.3 : 1}
...
const CustomLabel = ({ cx, cy, payload, t, hovered }) => {
  const isRecord = payload.value === 1000;
  const isHovered = hovered === payload.year;
  const dimmed = hovered && !isHovered;
  const color = isRecord ? '#EF4444' : t.accent;
  const minR = 7, maxR = 38, minV = 10, maxV = 1000;
  const r = minR + ((payload.value - minV) / (maxV - minV)) * (maxR - minR);
  const labelY = cy - r - 8;

  // Stagger labels to avoid overlap for closely-valued points
  // anchor right for 2026 (last point), left for first few, center otherwise
  const anchor = payload.year === 2026 ? 'end' : payload.year <= 2012 ? 'start' : 'middle';
  const xOffset = payload.year === 2026 ? -4 : payload.year <= 2012 ? 4 : 0;

  return (
    <g opacity={dimmed ? 0.2 : 1} style={{ transition: 'opacity 0.2s ease' }}>
      <text x={cx + xOffset} y={labelY}
        textAnchor={anchor} fill={color}
        fontSize={isRecord || isHovered ? 13 : 11}
        fontWeight={isRecord || isHovered ? 700 : 600}
        fontFamily="'Source Sans 3', system-ui">
        {payload.company}
      </text>
      <text x={cx + xOffset} y={labelY + 14}
...
export default function EuropeSeedChart() {
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <h1 style={{
          color: t.text, fontSize: 38, fontWeight: 900, letterSpacing: '-0.03em',
          lineHeight: 1.1, margin: 0, fontFamily: font,
        }}>
          $1B Seed for Ineffable Intelligence — New European Seed Level Unlocked
        </h1>
        <p style={{
          color: t.textSec, fontSize: 15, fontWeight: 400, margin: '12px 0 0',
          lineHeight: 1.5, fontFamily: font,
        }}>
          Largest seed round per year · 2010–2026
        </p>

        {/* Chart */}
        <div style={{ marginTop: 36 }}>
          <ResponsiveContainer width="100%" height={420}>
            <ScatterChart margin={{ top: 60, right: 40, bottom: 10, left: 10 }}>
              <CartesianGrid stroke={t.grid} vertical={false} />
              <XAxis
                dataKey="year"
                type="number"
                domain={[2009.5, 2026.5]}
                tickCount={17}
                tickFormatter={v => v % 1 === 0 ? String(v) : ''}
                tick={{ fill: t.textSec, fontSize: 12, fontFamily: font, fontWeight: 500 }}
              ...
              <YAxis
                ...
                type="number"
                domain={[0, 1100]}
                tick={{ fill: t.textMuted, fontSize: 12, fontFamily: font }}
                axisLine={false}
                tickLine={false}
                tickFormatter={v => `$${v}M`}
                width={70}
              ...
              <Scatter
                data={data}
                onMouseEnter={(d) => setHovered(d.year)}
                onMouseLeave={() => setHovered(null)}
                shape={(props) => {
                  const d = props.payload || props;
                  return <CustomDot cx={props.cx} cy={props.cy} payload={d} t={t} hovered={hovered} />;
                }}
                label={(props) => {
                  const d = props.viewBox ? data.find(x => x.year === props.value) : null;
                  return null;
                }}
              />
              {/* Render labels as a separate layer using foreignObject approach via SVG */}
              {data.map((d, i) => (
                <Scatter
                  key={`label-${i}`}
                  data={[d]}
                  shape={(props) => (
                    <CustomLabel cx={props.cx} cy={props.cy} payload={props.payload || d} t={t} hovered={hovered} />
                  )}
                  legendType="none"
                  tooltipType="none"
        ...
        <div style={{
          ...
          padding: '12px 20px', background: t.calloutBg,
          borderRadius: 10, border: `1px solid ${t.calloutBorder}`,
        }}>
          <span style={{ fontSize: 16 }}>🚀</span>
          <span style={{ color: t.text, fontSize: 14, fontFamily: font, lineHeight: 1.4 }}>
            <strong style={{ color: '#EF4444' }}>Ineffable Intelligence (2026)</strong> raised a record{' '}
            <strong style={{ color: '#EF4444' }}>$1B</strong> seed — 7.5× larger than any prior year's record.
          </span>
        </div>
