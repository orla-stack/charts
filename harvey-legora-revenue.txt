...
const data = [
  { age: 'Year 1', legora: 1.6, harvey: 1 },
  { age: 'Year 2', legora: 70, harvey: 50 },
  { age: 'Year 3', legora: null, harvey: 190 },
];
...
const themes = {
  dark: {
    ...
    logoVariant: 'white', dotStroke: '#001234',
  },
  light: {
    ...
    logoVariant: 'dark', dotStroke: '#FFFFFF',
  },
};
...
const CustomTooltip = ({ active, payload, label, t }) => {
  ...
  return (
    <div style={{
      ...
      <div style={{ fontWeight: 600, marginBottom: 8, fontSize: 14 }}>{label} after founding</div>
      {payload.map((p, i) => (
...
const CustomDot = ({ cx, cy, payload, dataKey, t, color, hover }) => {
  if (payload[dataKey] === null || payload[dataKey] === undefined) return null;
  const dimmed = hover && hover !== dataKey;
  return (
    <circle
      cx={cx} cy={cy} r={6}
      fill={color} stroke={t.dotStroke} strokeWidth={2.5}
...
const CustomLabel = ({ x, y, value, color, hover, dataKey }) => {
  if (value === null || value === undefined) return null;
  const dimmed = hover && hover !== dataKey;
  return (
    <text
      x={x} y={y - 16} textAnchor="middle"
...
export default function LegoraHarveyChart() {
    ...
    <div style={{
      ...
      <div style={{
        ...
        <h1 style={{
          ...
          Same Growth Curve, 1 Year Apart: Legora & Harvey
        </h1>
        <p style={{
          color: t.textSec, fontSize: 16, fontWeight: 400, margin: '12px 0 0',
          lineHeight: 1.5, fontFamily: font,
        }}>
          Annual revenue indexed by years since founding
        </p>

        {/* Legend */}
        <div style={{ display: 'flex', gap: 24, marginTop: 28, marginBottom: 8 }}>
          {[
            { key: 'legora', label: 'Legora (founded 2023)', color: t.accent },
            { key: 'harvey', label: 'Harvey (founded 2022)', color: t.accent2 },
          ].map(s => (
            <div
              key={s.key}
              style={{
            ...
            >
              ...
              <span style={{ color: t.textSec, fontSize: 14, fontWeight: 600, fontFamily: font }}>{s.label}</span>
            </div>
          ))}
        </div>
        ...
        <div style={{ marginTop: 8 }}>
          <ResponsiveContainer width="100%" height={360}>
            <LineChart data={data} margin={{ top: 30, right: 30, bottom: 10, left: 10 }}>
              ...
              <XAxis
                dataKey="age"
                tick={{ fill: t.textSec, fontSize: 14, fontFamily: font, fontWeight: 500 }}
              ...
              <Line
                ...
                strokeWidth={3}
                connectNulls={false}
                opacity={!hover || hover === 'legora' ? 1 : 0.15}
                dot={props => <CustomDot {...props} dataKey="legora" t={t} color={t.accent} hover={hover} />}
                label={props => <CustomLabel {...props} color={t.accent} hover={hover} dataKey="legora" />}
                style={{ transition: 'opacity 0.2s ease' }}
              />
              <Line
                type="monotone"
                ...
                strokeWidth={3}
                strokeDasharray="0"
                opacity={!hover || hover === 'harvey' ? 1 : 0.15}
                dot={props => <CustomDot {...props} dataKey="harvey" t={t} color={t.accent2} hover={hover} />}
                label={props => <CustomLabel {...props} color={t.accent2} hover={hover} dataKey="harvey" />}
                style={{ transition: 'opacity 0.2s ease' }}
              />
            </LineChart>
          </ResponsiveContainer>
